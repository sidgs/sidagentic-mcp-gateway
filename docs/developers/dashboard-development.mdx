# SAMI MCP Gateway dashboard frontend development

## 1. What the dashboard is

The Dashboard UI is served by the SAMI MCP Gateway Go server at `/` in development mode.
...
For the real SAMI MCP Gateway server, assets are embedded into the Go binary. The Go server does not read `web/dashboard/src` directly at runtime.
...
This starts SAMI MCP Gateway in development mode by default on `http://localhost:8080`.
...
If SAMI MCP Gateway is started with:
...
- `GET /api/dashboard/observability?range=24h|7d|30d&limit=10` — aggregated tool invocation metrics (by agent, top tools, tool groups, traffic)
- `GET /api/dashboard/lineage?range=7d` — graph nodes/edges linking agent apps, tool groups, servers, and tools
...
The browser frontend cannot do that, so the dashboard uses the SAMI MCP Gateway Go server as the OAuth callback receiver.
...
1. Start SAMI MCP Gateway in dev mode and open `http://localhost:8080/`.
...
- SAMI MCP Gateway is running in dev mode, not enterprise mode
...
4. Rebuild and run SAMI MCP Gateway.
