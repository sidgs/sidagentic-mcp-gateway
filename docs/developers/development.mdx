...
Mcpgateway consists of several main components:
...
For backward compatibility, Mcpgateway still reads existing `mcp.db` files if they exist, but new databases are created as `sami-mcp-gateway.db`.
