...
description: "Manage machine identities for MCP clients and user accounts for human operators in Mcpgateway enterprise mode."
---
...
- **Users** represent humans who need authenticated access to Mcpgateway.
...
Dashboard users authenticate via OIDC or platform JWT. Roles and teams are stored in the gateway database and enforced on `/dashboard/*` routes. See [RBAC roles and teams](/governance/rbac-and-teams) for the full model.

Users are human accounts for operators who need authenticated CLI or API access.

Compared with admins, a standard user has a narrower permission set. They can inspect and use Mcpgateway, but they do not get the same write capabilities as an admin.

## Create users

```bash
# Auto-generated token
sami-mcp-gateway create user bob

# Custom token
sami-mcp-gateway create user alice --access-token alice_token_123

# From config file
sami-mcp-gateway create user --conf /path/to/user-config.json
```
...
- create one user in sami-mcp-gateway for each human operator who needs CLI or API access
...
<CardGroup cols={2}>
  ...
  <Card title="Production deployment" icon="cloud" href="/deployment/production">
    Initialize and operate Mcpgateway correctly in enterprise mode.
  </Card>
</CardGroup>
