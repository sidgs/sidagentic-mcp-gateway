...
description: "Understand Mcpgateway's enterprise operating model: authenticated clients, per-client server access, user accounts, and governance for shared deployments."
---
...
The Governance model is useful for organizations running sami-mcp-gateway in a shared environment in the **enterprise mode**.
...
Mcpgateway uses three main identity and access concepts in enterprise mode.
...
Users are human identities for people who need authenticated access to Mcpgateway. They have a narrower permission set than admins.

## Typical enterprise flow

<Steps>
  <Step title="Start Mcpgateway in enterprise mode">
    Deploy the gateway in enterprise mode and connect it to PostgreSQL.
  </Step>
  <Step title="Initialize the server">
    Run `sami-mcp-gateway init-server` once to create the initial admin identity.
  </Step>
  ...
  <Step title="Create users when needed">
    Create standard users for human operators who need authenticated visibility into Mcpgateway.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Upstream authentication" icon="key" href="/governance/upstream-authentication">
    Configure bearer tokens and custom headers for Mcpgateway's upstream server connections.
  </Card>
  ...
  <Card title="Production deployment" icon="cloud" href="/deployment/production">
    Run Mcpgateway with the expected enterprise operating model.
  </Card>
</CardGroup>
