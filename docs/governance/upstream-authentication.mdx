...
description: "Configure sami-mcp-gateway to make authenticated requests to upstream MCPs"
---

This page is about **upstream authentication**: how Mcpgateway authenticates when it calls an upstream MCP server.
...
Mcpgateway now supports OAuth flows for upstream HTTP MCP servers in beta.
...
1. Mcpgateway first tries a normal unauthenticated registration.
...
4. Mcpgateway stores the resulting token and completes server registration.
...
If `oauth_client_id` is omitted, Mcpgateway attempts dynamic client registration when the upstream authorization server supports it.
...
Placeholder expansion happens in the CLI process before the config is sent to Mcpgateway.
...
Mcpgateway will start the STDIO server process with those environment variables, allowing the upstream server to read its credentials from the process environment.

## Important scope distinction

Two different authentication layers can exist in the same deployment:

- **Gateway authentication**: Enterprise-mode clients authenticating to Mcpgateway.
- **Upstream authentication**: Mcpgateway authenticating to a registered MCP server.
...
<CardGroup cols={2}>
  ...
  <Card title="Access control" icon="shield-halved" href="/governance/access-control">
    Configure how MCP clients authenticate to the Mcpgateway gateway itself.
  </Card>
