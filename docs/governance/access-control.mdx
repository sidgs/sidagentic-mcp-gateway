...
```bash
sami-mcp-gateway create mcp-client cursor-local --allow "calculator, github"
```

Mcpgateway prints the generated access token once:
...
<Tip>
  ...
  ```bash
  sami-mcp-gateway create mcp-client cursor-local --allow "*"
  ```
...
```bash
sami-mcp-gateway create mcp-client cursor-local --allow "calculator, github" --access-token my_custom_token
```
...
```bash
sami-mcp-gateway create mcp-client --conf ./cursor-local.json
```

<Warning>
  When creating an MCP client from a config file, you must provide a token explicitly. Mcpgateway cannot print an auto-generated token back through the file-based workflow.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Production deployment" icon="cloud" href="/deployment/production">
    Run Mcpgateway in enterprise mode and initialize it correctly.
  </Card>
