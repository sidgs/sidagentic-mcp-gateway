...
description: "Use OpenTelemetry to monitor sami-mcp-gateway and all MCP interactions in your infrastructure."
---

SAMI MCP Gateway emits Prometheus-compatible metrics using OpenTelemetry and exposes them at the `/metrics` endpoint.
...
Once SAMI MCP Gateway is running with telemetry enabled:
...
|---|---|---|
| `sami_mcp_gateway_tool_calls_ratio_total` | Counter | Total number of tool calls handled by SAMI MCP Gateway. |
| `sami_mcp_gateway_tool_call_latency_seconds` | Histogram | Latency for tool calls, in seconds. |
...
```text
# HELP sami_mcp_gateway_tool_calls_ratio_total Total number of tool calls
# TYPE sami_mcp_gateway_tool_calls_ratio_total counter
sami_mcp_gateway_tool_calls_ratio_total{mcp_server_name="calculator",tool_name="add",outcome="success"} 42

# HELP sami_mcp_gateway_tool_call_latency_seconds Latency of tool calls in seconds
# TYPE sami_mcp_gateway_tool_call_latency_seconds histogram
sami_mcp_gateway_tool_call_latency_seconds_count{mcp_server_name="calculator",tool_name="add",outcome="success"} 42
```

## Prometheus

The production Compose path ships with a Prometheus service already configured to scrape SAMI MCP Gateway:

```yaml
scrape_configs:
  - job_name: 'sami-mcp-gateway'
    static_configs:
      - targets: ['sami-mcp-gateway:8080']
...
## Feedback
If you want to see more metrics or have feedback on the current ones, please share your thoughts in the  or open an .

## Related pages

<CardGroup cols={2}>
  <Card title="Production deployment" icon="cloud" href="/deployment/production">
    Run SAMI MCP Gateway in enterprise mode with the right operational defaults.
  </Card>
