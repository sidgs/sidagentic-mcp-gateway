...
Cursor supports the streamable HTTP MCP transport natively, so you can point it directly at the Mcpgateway gateway. Once configured, Cursor's AI agent can discover and invoke every tool registered in Mcpgateway.
...
<Steps>
  ...
  <Step title="Save and reload">
    Save the configuration. Cursor will connect to Mcpgateway and load the available tools automatically.
  </Step>
  ...
  <Step title="Test the connection">
    ...
    The agent will query Mcpgateway and return all tools from your registered servers.
  </Step>
</Steps>

## Enterprise mode (authentication)

If your Mcpgateway instance runs in enterprise mode, create a dedicated MCP client for Cursor and include its access token in the request headers.
