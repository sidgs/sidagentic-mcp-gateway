...
description: "List, inspect, invoke, enable, and disable the tools and prompts exposed by the MCP servers registered in Mcpgateway."
---

Once an MCP server is registered, Mcpgateway tracks what it exposes and lets you manage that surface area without removing the server itself.
...
```bash
sami-mcp-gateway list tools --server context7
```
...
Mcpgateway supports [MCP Prompts](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts). If a registered server exposes prompts, Mcpgateway registers them alongside that server's tools.
...
sami-mcp-gateway list prompts --server huggingface
```
...
```bash
sami-mcp-gateway get prompt "huggingface__Model Details" --arg model_id="openai/gpt-oss-120b"
```
...
Mcpgateway lets you hide tools and prompts from clients without deleting the underlying server registration.

When you disable something:

- it disappears from gateway discovery
- MCP clients cannot call it through Mcpgateway
...
```bash
sami-mcp-gateway disable tool context7__get-library-docs
sami-mcp-gateway enable tool context7__get-library-docs
```

### Disable or re-enable all tools from one server

```bash
sami-mcp-gateway disable tool context7
sami-mcp-gateway enable tool context7
```
...
```bash
sami-mcp-gateway disable server context7
sami-mcp-gateway enable server context7
```

### Disable or re-enable prompts

```bash
sami-mcp-gateway disable prompt "huggingface__Model Details"
sami-mcp-gateway enable prompt "huggingface__Model Details"

sami-mcp-gateway disable prompt context7
sami-mcp-gateway enable prompt context7
```
