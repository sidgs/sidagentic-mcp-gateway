...
As more MCP servers are added to sami-mcp-gateway, the number of tools, prompts and resources available through the gateway mcp endpoint can quickly explode.

This can cause context overload for your clients.

Mcpgateway allows you to create custom MCP endpoints that only expose cherry-picked tools to clients. These are called **Tool Groups**.
...
When you create a group, Mcpgateway registers a new streamable HTTP endpoint at:
...
Mcpgateway also creates SSE endpoints for the group:
...
<Warning>
  SSE is a deprecated MCP transport and its support in Mcpgateway is limited. Prefer the streamable HTTP endpoint unless you specifically need SSE compatibility.
</Warning>
...
<Tabs>
  <Tab title="Cherry-pick specific tools">
    ...
    This group exposes exactly three tools regardless of how many tools are registered in Mcpgateway.

    <Tip>
      Run `sami-mcp-gateway list tools` to see all available tool names before building your group configuration.
    </Tip>
  </Tab>
...
```bash
sami-mcp-gateway create group -c ./claude-tools-group.json
```

Mcpgateway confirms the group was created and prints its endpoint:
...
<Steps>
  ...
  <Step title="Inspect a specific group">
    ```bash
    sami-mcp-gateway get group claude-tools
    ```
  ...
  <Step title="Delete a group">
    ```bash
    sami-mcp-gateway delete group claude-tools
    ```
...
sami-mcp-gateway list tools --group claude-tools

# Invoke a tool through the group context
sami-mcp-gateway invoke filesystem__read_file --group claude-tools --input '{"path": "README.md"}'
```
...
<AccordionGroup>
  <Accordion title="Prompts & Resources are currently not supported in Tool Groups">
    [MCP Prompts](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts) and Resources are not currently available through group endpoints. Clients connected to a group endpoint cannot discover or invoke prompts. This is a known issue tracked at .
  </Accordion>
  <Accordion title="Enterprise mode: only admins can create groups">
    When Mcpgateway runs in `enterprise` mode, only an admin account can create Tool Groups. Standard users do not have permission to create groups. Support for user-scoped groups is planned for a future release.
  </Accordion>
</AccordionGroup>
