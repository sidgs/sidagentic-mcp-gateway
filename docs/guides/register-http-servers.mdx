...
description: "Register remote MCP servers in sami-mcp-gateway using CLI. Expose them via the mcp gateway."
---

MCP servers using the [Streamable HTTP transport](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#streamable-http) can be added to Mcpgateway.

Once registered, a server's tools, prompts, and resources are available to any MCP client that connects to sami-mcp-gateway mcp endpoint.
...
```bash
sami-mcp-gateway register --name calculator --description "Provides some basic math tools" --url http://127.0.0.1:5000/mcp
```

<Note>
If Mcpgateway is running inside Docker on macOS or Windows, the container cannot reach `127.0.0.1` on your host. Use `host.docker.internal` instead:

```bash
sami-mcp-gateway register --name calculator --description "Provides some basic math tools" --url http://host.docker.internal:5000/mcp
```
</Note>
...
```bash
sami-mcp-gateway register -c ./deepwiki.json
```
...
| `bearer_token` | No       | If set, Mcpgateway adds `Authorization: Bearer <token>` to every request to this server.                                      |
| `oauth_redirect_uri` | No | Redirect URI to use if the upstream server requires OAuth. When using the CLI interactively, Mcpgateway can usually generate a localhost callback automatically. |
...
If the upstream MCP server responds with `401 Unauthorized` and advertises OAuth, Mcpgateway can complete that authorization flow during registration.

Typical CLI flow:

```bash
sami-mcp-gateway register --name todoist --url https://ai.todoist.net/mcp
```

What happens next:

1. Mcpgateway attempts a normal registration first.
2. If the upstream server requires OAuth, the CLI opens your browser.
3. You approve access.
4. Mcpgateway stores the resulting upstream token and completes registration automatically.
...
After registering a server, inspect what Mcpgateway discovered:

```bash
sami-mcp-gateway list tools

# get the usage schema of a tool
sami-mcp-gateway usage deepwiki__read_wiki_structure

# call a tool from cli
sami-mcp-gateway invoke deepwiki__read_wiki_structure --input '{"repoName": "facebook/react"}'
```

That is the quickest way to confirm both the upstream server and Mcpgateway routing are working as expected.

## Deregister a server

To remove a server from Mcpgateway:

```bash
sami-mcp-gateway deregister deepwiki
```

Deregistering removes the server and all the tools, prompts, and resources it provided. They are no longer accessible by clients using sami-mcp-gateway.
...
<CardGroup cols={2}>
  ...
  <Card title="Connect Claude Desktop" icon="plug" href="/integrations/claude">
    Connect Claude Desktop to Mcpgateway and start using your registered tools through one MCP endpoint.
  </Card>
