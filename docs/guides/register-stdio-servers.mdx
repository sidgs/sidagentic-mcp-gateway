...
description: "Register stdio transport-based MCP servers in sami-mcp-gateway using CLI. Expose them via the mcp gateway."
---

Mcpgateway supports MCP servers that use the [STDIO transport](https://modelcontextprotocol.io/specification/2025-03-26/basic/transports#stdio).

These servers run as child processes on the same host as the Mcpgateway server and communicate through standard input and output. Common examples include [filesystem](https://github.com/modelcontextprotocol/servers/tree/main/src/filesystem) and [time](https://github.com/modelcontextprotocol/servers/tree/main/src/time).
...
<Steps>
  ...
  <Step title="Register with Mcpgateway">
    Pass the file to the `register` command with the `-c` flag:

    ```bash
    sami-mcp-gateway register -c ./filesystem.json
    ```

    Mcpgateway spawns the process, initializes the connection, loads its tool list, and registers the server.
  </Step>
</Steps>
...
| `args`        | No       | List of arguments passed to `command` when Mcpgateway spawns the process.                      |
...
When Mcpgateway runs inside a Docker container, the container process does not have access to your host filesystem by default. To use the `filesystem` MCP server from inside Docker, you need to mount the host directory you want to expose.

The default `docker-compose.yaml` provided by Mcpgateway mounts your current working directory as `/host` inside the container. Use `/host` as the target directory in your config:
...
STDIO-based servers that rely on `npx` or `uvx` require those tools to be present in the Mcpgateway container.

<Warning>
The default `sami-mcp-gateway` Docker image is minimal and does not include `npx` or `uvx`. You must use the `latest-stdio` tagged image when registering STDIO servers that use these commands.
</Warning>

Start Mcpgateway with the `stdio` image using the `MCPGATEWAY_IMAGE_TAG` environment variable:
...
<Note>
If you are using the development `docker-compose.yaml`, the `latest-stdio` tag is already the default. You only need to set `MCPGATEWAY_IMAGE_TAG` explicitly when using `docker-compose.prod.yaml`.
</Note>

If your STDIO server depends on a tool other than `npx` or `uvx`, create a custom Docker image that builds on the Mcpgateway base image and installs the additional dependency.

## Debugging STDIO servers

If a STDIO server fails to start or throws errors during a tool call, check the server logs. The server captures `stderr` output from child processes and writes it to its own log stream, making it the primary place to diagnose STDIO server issues.
