...
Mcpgateway ships as a single binary.

You will usually use this binary in two ways:

- To run the Mcpgateway gateway server, which manages all upstream MCPs and serves the HTTP API & MCP endpoints
...
<Tabs>
  ...
  <Tab title="Docker">
    Pull the official Mcpgateway image if you want to run the gateway in Docker:
...
By default, the CLI connects to Mcpgateway at `http://127.0.0.1:8080`. If your server runs on a different host or port, configure the registry URL in one of two ways:
