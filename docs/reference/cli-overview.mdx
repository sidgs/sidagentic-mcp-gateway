...
description: "Information about how to use the sami-mcp-gateway CLI"
---

The `sami-mcp-gateway` binary serves two roles: it runs the gateway server and acts as a client for managing everything in that server.
...
|------|------|---------|-------------|
| `--registry` | string | `http://127.0.0.1:8080` | Base URL of the Mcpgateway registry server. |
...
On startup, the CLI attempts to read `.sami-mcp-gateway.conf` in your home directory.

The file is YAML and contains the following fields:

```yaml ~/.sami-mcp-gateway.conf
...
| Field | Description                                                                                    |
|-------|------------------------------------------------------------------------------------------------|
| `registry_url` | URL of the Mcpgateway server. Overrides the `--registry` default.                               |
| `access_token` | Auth token that represents the user, sent with every CLI request. Required in enterprise mode only. |

In enterprise mode, this file gets created automatically when you initialise sami-mcp-gateway or login using your CLI.
