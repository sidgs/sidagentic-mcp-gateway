...
description: "Reference for creating, inspecting, updating, and deleting tool groups from the sami-mcp-gateway CLI."
---
...
<Note>
  Treat `sami-mcp-gateway create group --help`, `get group --help`, and `update group --help` as the authoritative source for current flags. This page focuses on the important workflows and config shape.
</Note>
...
```bash
sami-mcp-gateway create group --conf <file>
```
...
```bash
sami-mcp-gateway create group --conf ./engineering-tools.json
```

After creation, sami-mcp-gateway serves the group on:
...
<Warning>
  SSE support exists for compatibility, but it is a deprecated transport with limited support in sami-mcp-gateway. Prefer the streamable HTTP endpoint.
</Warning>
...
```bash
sami-mcp-gateway get group <name>
```

Example:

```bash
sami-mcp-gateway get group engineering-tools
```
...
```bash
sami-mcp-gateway update group --conf <file>
```

Example:

```bash
sami-mcp-gateway update group --conf ./engineering-tools.updated.json
```
...
```bash
sami-mcp-gateway delete group <name>
```

Example:

```bash
sami-mcp-gateway delete group engineering-tools
```
