...
description: "Reference for ~/.sami-mcp-gateway.conf"
---

The Mcpgateway CLI reads a YAML configuration file at `~/.sami-mcp-gateway.conf` on startup. It stores two values: the URL of the Mcpgateway server and an access token for authenticated requests. Most users never edit this file by hand — `sami-mcp-gateway init-server` and `sami-mcp-gateway login` write to it automatically.

## Client config file (`~/.sami-mcp-gateway.conf`)
...
<ParamField path="registry_url" type="string" default="http://127.0.0.1:8080">
  Base URL of the Mcpgateway server the CLI connects to. Set this to avoid passing `--registry` on every command.
...
<ParamField path="access_token" type="string">
  Bearer token sent in the `Authorization: Bearer` header with every API request. Required when the server runs in enterprise mode. Populated automatically by `sami-mcp-gateway init-server` (admin token) or `sami-mcp-gateway login` (user token).
...
2. `registry_url` set in `~/.sami-mcp-gateway.conf`
...
`~/.sami-mcp-gateway.conf` is usually created or updated in one of two ways:

- `sami-mcp-gateway init-server` writes the initial admin credentials during enterprise bootstrap
- `sami-mcp-gateway login <user-token>` stores a user token for later authenticated CLI use
...
Several `sami-mcp-gateway` commands accept a `-c` / `--conf` flag pointing to a JSON file instead of inline CLI flags. This section documents every supported JSON format.
...
Used with `sami-mcp-gateway register -c <file>`.
...
|---|---|---|---|
| `name` | string | Yes | Unique name for this server in Mcpgateway. |
| `transport` | string | Yes | Must be `"streamable_http"`. |
| `description` | string | No | Human-readable description. |
| `url` | string | Yes | Full URL of the MCP server endpoint. |
| `bearer_token` | string | No | If set, Mcpgateway adds `Authorization: Bearer <token>` to every upstream request. |
...
<Note>
  ...
  For interactive CLI use, you usually do not need to set `oauth_redirect_uri` manually. Mcpgateway can provision a localhost callback automatically when the upstream MCP server actually requires OAuth.
</Note>
...
### Register a REST / OpenAPI server

Used with `sami-mcp-gateway register -c <file>` when exposing a REST API (not an MCP protocol server) through Mcpgateway.
...
```json
{
  ...
  "transport": "rest",
  "description": "Petstore REST API exposed as MCP tools",
  "base_url": "https://petstore.example.com",
  "openapi_spec_url": "https://petstore.example.com/openapi.json",
  "excluded_operations": ["deletePet"],
  "rest_auth": {
    "type": "api_key",
    "api_key_header": "X-API-Key",
    "api_key_value": "${API_TOKEN}"
  }
}
```
...
|---|---|---|---|
| `name` | string | Yes | Unique name for this server in Mcpgateway. |
| `server_kind` | string | Yes | Must be `"rest_openapi"` or `"rest_endpoint"`. |
| `transport` | string | Yes | Must be `"rest"`. |
| `base_url` | string | Yes | Base URL of the upstream REST API. |
| `openapi_spec_url` | string | Conditional | URL to fetch the OpenAPI 3 spec (`rest_openapi` only). |
| `openapi_spec` | string | Conditional | Inline OpenAPI 3 YAML/JSON (`rest_openapi` only). Provide exactly one of `openapi_spec_url` or `openapi_spec`. |
| `excluded_operations` | string array | No | OpenAPI `operationId` values to skip when synthesizing tools. |
| `rest_auth.type` | string | No | Upstream auth: `none`, `api_key`, `basic`, `bearer`, or `oauth` (default `none`). |
| `rest_auth.api_key_header` | string | No | Header name for API key auth. |
| `rest_auth.api_key_query` | string | No | Query parameter name for API key auth. |
| `rest_auth.api_key_value` | string | No | API key value. |
| `rest_auth.username` / `rest_auth.password` | string | No | Basic auth credentials. |
| `bearer_token` | string | No | Shorthand for bearer auth when `rest_auth.type` is omitted. |

For a single manually defined REST operation, use `server_kind: "rest_endpoint"` with `method`, `path`, `tool_name`, `tool_description`, and `parameters`.

REST servers appear in tool groups the same way as MCP protocol servers (`included_servers`, `included_tools`).

### Create a tool group

Used with `sami-mcp-gateway create group -c <file>`.
...
Used with `sami-mcp-gateway create mcp-client --conf <file>` (enterprise mode).
...
```json
{
  ...
  "access_token_ref": {
    "file": "/run/secrets/sami_mcp_gateway_token",
    "env": "MCPGATEWAY_CLIENT_TOKEN"
  }
}
```
...
<Warning>
  When creating a client from a config file, you must supply a token via `access_token` or `access_token_ref`. Mcpgateway cannot print a generated token back to the console when reading from a file.
</Warning>
...
Used with `sami-mcp-gateway create user --conf <file>` (enterprise mode).
...
The same `${VAR_NAME}` placeholder substitution applies to user config files. When using a config file you must supply a token — Mcpgateway cannot print a generated one to the console.
