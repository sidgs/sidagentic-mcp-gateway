...
The Mcpjungle CLI reads a YAML configuration file at `~/.sami-mcp-gateway.conf` on startup. It stores two values: the URL of the Mcpjungle server and an access token for authenticated requests. Most users never edit this file by hand — `sami-mcp-gateway init-server` and `sami-mcp-gateway login` write to it automatically.
