...
Initializes the Mcpgateway server when running in enterprise mode. Run it once, from your local machine, after the server has started.

```bash
sami-mcp-gateway init-server [--registry <url>]
```

`init-server` creates the first admin user and stores the registry URL and token in `~/.sami-mcp-gateway.conf`, authenticating your CLI as admin.
